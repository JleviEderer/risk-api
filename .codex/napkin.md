...
8. **[2026-07-06] A healthy Augur app can still be dead-linked in CDP/Bazaar**
   Do instead: check CDP/Bazaar discovery directly with `scripts/check_cdp_discovery.py`, `/discovery/merchant?payTo=...`, and `/discovery/search?urlSubstring=...` before treating discovery as fixed; on 2026-07-06, live `augurrisk.com` passed CDP validate and a fresh paid settlement succeeded, but CDP still only surfaced `https://risk-api.life.conway.tech/analyze`, so use `docs/CDP_BAZAAR_ESCALATION_2026-07-06.md` if the delayed index does not update.
