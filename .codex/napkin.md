...
1. **[2026-06-04] Keep public `/stats` bounded over durable SQLite**
   Do instead: keep exact all-time totals, funnel stages, and traffic classes on indexed SQLite queries; bound display-only aggregates to the latest 20,000 events and the hourly chart to 7 days, read `raw_json` only for the 20 recent rows, and keep JSONL as a fallback only when `ANALYTICS_DB_PATH` is unset so `/stats` cannot break `/dashboard` and production does not dual-write every request; never bulk-backfill old analytics rows inside a public request on the 512 MB Fly VM.
...
9. **[2026-07-08] Decision-primary responses need serializer and surface invariants**
   Do instead: keep top-level `decision` as the primary branch field, always emit `contract_decision`, rebuild top-level `recommended_policy.action` to equal `decision`, keep `action_evaluation` unchanged, and verify OpenAPI/examples/MCP structured output so public surfaces do not strip or contradict those fields.
