...
- Deployment completed on 2026-08-28 from commit `16e31ce`: GitHub Typecheck and Public Auto Bench passed, the guarded Fly Deploy workflow passed, and live sequential checks returned `/health` `200`, `/stats` `200` in about `0.44s`, and `/dashboard` `200`. Live `/stats` reported durable SQLite at `/data/analytics.sqlite3`, `537,419` total requests, `39` paid requests, `169` hourly buckets, and `20` recent rows.
- Analytics capacity measurement: Fly volume `augur_analytics` is mounted at `/data` and provisioned as 1 GB. After remediation on 2026-08-28 it used `408 MB` with `500 MB` free (`45%`), so the goal's less-than-25%-free expansion threshold was not met and the volume was not resized. `/data` contained the canonical `analytics.sqlite3` at about `408 MB`, its small WAL/shared-memory files, and no `requests.jsonl`. Production overrides the repo's JSONL path with `REQUEST_LOG_PATH=/tmp/requests.jsonl`; that ephemeral duplicate was about `611 KB` and `1,996` lines before the SQLite-primary logging deploy. No canonical SQLite data or `paid_response_snapshots` were deleted, truncated, or rewritten.
- SQLite-primary logging deployed from commit `e039e8a` to Fly machine version `134`. When `ANALYTICS_DB_PATH` is configured, the app now leaves `REQUEST_LOG_PATH` as an inactive fallback instead of attaching a second JSONL writer; JSONL-only environments retain the old behavior. Local validation passed with `431` tests, Pyright `0 errors`, targeted `py_compile`, and a successful production-sized stats reproducer. GitHub Typecheck, Public Auto Bench, and Fly Deploy passed. Live sequential checks returned `/health`, `/stats`, and `/dashboard` `200`; the two logged verification requests advanced SQLite from `539,438` to `539,440`, while `/tmp/requests.jsonl` remained absent. Final read-only inspection showed `539,443` request events and all `4` paid-response snapshots intact. Rollback instructions are in `docs/DURABLE_ANALYTICS_CUTOVER.md`.
- Recovery evidence: the prior `/tmp/requests.jsonl` disappeared when Fly replaced the ephemeral root filesystem during deploy. Its exact machine-version window was reconstructed read-only from SQLite `raw_json` as `2,014` rows and `630,818` bytes, then downloaded to `.codex/live_db/2026-08-28-jsonl-remediation/requests.reconstructed.jsonl`; SHA-256 is `9f4e06b2a3e9baa9ae47bc66463ca763671025da0d8839b22b5cf5826fb29762`. Fresh Fly volume snapshot `vs_4ov1Oz98KDaFkgJQQxgQXPn` reached `created` at `456 MiB`, covering the canonical SQLite volume.
- Security follow-up: rotate the production `CDP_API_KEY_SECRET` and `BASESCAN_API_KEY`, then update the corresponding Fly secrets. Their values were unintentionally exposed in terminal output during the 2026-08-28 runtime inspection; do not record the values in repo files or chat.
...
- A-003 implementation: completed and deployed on 2026-07-08 from `docs/A003_DESIGN_SPEC.md` v2. Public `/analyze` serialization now always emits `contract_decision`, makes top-level `decision` the effective max-strictness branch field, and rebuilds top-level `recommended_policy.action` to equal the emitted `decision`. `action_context` and `action_evaluation` remain unchanged. Public first-party docs/OpenAPI/examples and the in-repo MCP wrapper were updated; the MCP output schema now exposes `decision`, `contract_decision`, and `recommended_policy` instead of stripping them from structured output. Validation passed: `python -m pytest -q` -> `429 passed`; `python -m pyright src\ tests\` -> `0 errors`; `python auto\loop.py` -> `PASS (59/59 checks passed)`; targeted `py_compile` passed; `npm run smoke` in `examples/javascript/augur-mcp` passed without paid credentials; GitHub Typecheck and Fly Deploy passed for implementation commit `51bedaa`. Live checks passed for `/health`, unpaid WETH `/analyze` returning `402`, lowercase `payment-required` header, OpenAPI `contract_decision` schema/required field, and `/llms-full.txt` `contract_decision`/primary-decision prose. Fable review approved with no blocking findings. Paid approve smoke on 2026-07-08 used the configured local payer wallet against WETH approve and returned `200`. Fresh Fly SQLite pull `.codex/live_db/2026-07-08-1610/analytics.sqlite3` showed `37` paid `/analyze` 200 rows and `2` paid response snapshots; latest snapshot was untruncated (`response_bytes=797`) with `decision=warn`, `contract_decision=allow`, top-level policy action `warn`, `action_evaluation.decision=warn`, `action=approve`, and no payment/private markers. Paid MCP smoke passed and returned structured content containing `decision`, `contract_decision`, and `recommended_policy`. `tests/fixtures/paid_contract_cases.json` and `tests/test_paid_contract_regressions.py` were not edited.
- Pricing decision: on 2026-07-10 the user chose to run a `$0.02` x402 adoption/pricing test rather than keep `$0.10`. This has not been implemented yet. Treat it as a bounded P-001 pass: update the enforced production `PRICE` secret to `$0.02` (`20000` USDC base units), update first-party copy and machine-readable surfaces, update x402.jobs registration metadata/scripts, verify live 402 requirements show `$0.02`, run one paid smoke if credentials/funds are available, and document success criteria plus rollback rule before/with the change. Do not change detectors, scoring, response shape, payment wallet, canonical URL, or paid-response logging privacy limits as part of the pricing pass.
- A-003 close-out (2026-07-10): Fable's implementation review approved `51bedaa`/`2edceed` with zero blocking findings after independent verification (serializer §4 semantics incl. the rebuilt-policy guard, full test matrix, zero fixture edits, live OpenAPI/402/llms re-checks). With the paid approve smoke, snapshot row, and paid MCP smoke recorded in `f27f163`, A-003 is DONE end to end: spec `800707d` → critique → revision `d5f1a81` → implementation `51bedaa` → review → paid verification. The Fable-specs/Codex-critiques-and-implements/Fable-reviews split is the default for future response-contract changes (including the P-001 pricing pass if it touches copy surfaces).
...
- Next exact tasks:
  1. Run the P-001 `$0.02` pricing-test pass, including live 402 verification and x402.jobs update.
  2. Recheck CDP/Bazaar on the next support-response window or after 2026-07-09.
  3. Monitor post-A-003 paid rows using `paid_response_snapshots` and decide whether `L-002` needs facilitator IDs/tx hashes or whether off-chain Blockscout correlation is enough.
  4. Narrow distribution (`O-001` first slice) is unblocked once P-001 lands: x402.jobs listing is live and canonical, the CDP escalation is submitted, and the decision-primary story plus WETH proof are ready to show. Sequence it after the price test so outreach quotes the tested price. Broad distribution still waits on CDP/Bazaar resolution.
...
- Confirmed a real paid production smoke test on 2026-04-03 using the Conway wallet against the live deployed agent wallet:
  ...
  - payer wallet: configured local Conway wallet, address intentionally omitted from this handover
