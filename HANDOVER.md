...
- Analytics capacity measurement: Fly volume `augur_analytics` is mounted at `/data` and provisioned as 1 GB. On 2026-08-28 it used `410 MB` with `499 MB` free (`46%`), so the goal's less-than-25%-free expansion threshold was not met and the volume was not resized. `/data` contained the canonical `analytics.sqlite3` at about `408 MB`, a roughly `2 MB` WAL, its small shared-memory file, and no `requests.jsonl`. Production overrides the repo's JSONL path with `REQUEST_LOG_PATH=/tmp/requests.jsonl`; that ephemeral duplicate was about `611 KB` and `1,996` lines before the SQLite-primary logging deploy. No canonical SQLite data or `paid_response_snapshots` were deleted, truncated, or rewritten.
